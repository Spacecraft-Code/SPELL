...
public abstract class AbstractJJTGrammarState implements IJJTGrammarState
{
	...
	protected AbstractJJTGrammarState()
	{
	...
	public SimpleNode setNodePos()
	{
		...
		if (n.beginLine == 0) n.beginLine = popLine;

		int popCol = columns.pop();
		if (n.beginColumn == 0) n.beginColumn = popCol;
		return n;
	}
	...
	public void closeNodeScope(Node n, int num) throws ParseException
	{
		...
		if (newNode == null) { throw new ParseException(
	...
	public void closeNodeScope(Node n, boolean condition) throws ParseException
	{
		...
		if (condition)
		{
			...
			if (newNode == null) { throw new ParseException(
			        "Internal AST builder error when closing node:" + sn); }
			if (marks.size() > 0)
			{
				mk = marks.pop();
			}
