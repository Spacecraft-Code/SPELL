...
/**
 * This is a custom string buffer optimized for append(), clear() and
 * deleteLast().
 * 
 * Basically it aims at being created once, being used for something, having
 * clear() called and then reused (ultimately providing minimum
 * allocation/garbage collection overhead for that use-case).
 * 
 * append() is optimizing by doing less checks (so, exceptions thrown may be
 * uglier on invalid operations and null is not checked for in the common case
 * -- use appendObject if it may be null).
 * 
 * clear() and deleteLast() only change the internal count and have almost zero
 * overhead.
 * 
 */
public final class FastStringBuffer
{
	 ...
	 * 
	 * @param s
	 *            string with the initial contents
	 * @param additionalSize
	 *            the additional size for the buffer
	 */
	...
	/**
	 * Resizes the internal buffer to have at least the minimum capacity passed
	 * (but may be more)
	 */
	...
	{
		if (start < 0) throw new StringIndexOutOfBoundsException(start);
		if (start > count) throw new StringIndexOutOfBoundsException(
		        "start > length()");
		if (start > end) throw new StringIndexOutOfBoundsException(
		        "start > end");
		if (end > count) end = count;
