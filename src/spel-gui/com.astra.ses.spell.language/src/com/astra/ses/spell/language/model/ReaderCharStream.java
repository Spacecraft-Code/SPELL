 ...
 * Reader. This file started life as a copy of ASCII_CharStream.java.
 */
...
public final class ReaderCharStream implements CharStream
{
	...
	private final void UpdateLineColumn(char c)
	{
		...
		case '\n':
			prevCharIsLF = true;
			break;
		// ok, this was commented out because the position would not reflect
		// correctly the positions found in the ast.
		// this may have other problems, but they have to be analyzed better to
		// see the problems this may bring
		// (files that mix tabs and spaces may suffer, but I could not find out
		// very well the problems -- anyway,
		// restricting the analysis to files that have only tabs or only spaces
		// seems reasonable -- shortcuts are available
		// so that we can convert a file from one type to another, so, what
		// remains is making some lint analysis to be sure of it).
		// case '\t' :
		// column--;
		// column += (8 - (column & 07));
		// break;
	...
	public final char readChar() throws java.io.IOException
	{
		...
		{
			System.out
			        .println("ReaderCharStream: readChar >>" + (int) c + "<<");
		}
		return (c);
	}
	...
	{
		String s = null;
		if (bufpos >= tokenBegin) s = new String(buffer, tokenBegin, bufpos
		        - tokenBegin + 1);
		else s = new String(buffer, tokenBegin, bufsize - tokenBegin)
	...
	public final char[] GetSuffix(int len)
	{
		...
		if ((bufpos + 1) >= len) System.arraycopy(buffer, bufpos - len + 1,
		        ret, 0, len);
		else
		{
		...
		if (DEBUG)
		{
			System.out.println("ReaderCharStream: GetSuffix:" + len + " >>"
			        + new String(ret) + "<<");
		}
