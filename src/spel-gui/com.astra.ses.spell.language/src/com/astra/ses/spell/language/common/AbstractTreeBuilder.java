...
import com.astra.ses.spell.language.model.ast.AugAssign;
import com.astra.ses.spell.language.model.ast.BinOp;
import com.astra.ses.spell.language.model.ast.BoolOp;
import com.astra.ses.spell.language.model.ast.Break;
import com.astra.ses.spell.language.model.ast.Compare;
import com.astra.ses.spell.language.model.ast.Continue;
import com.astra.ses.spell.language.model.ast.Delete;
import com.astra.ses.spell.language.model.ast.Exec;
import com.astra.ses.spell.language.model.ast.Expr;
import com.astra.ses.spell.language.model.ast.ExtSlice;
import com.astra.ses.spell.language.model.ast.For;
import com.astra.ses.spell.language.model.ast.If;
...
import com.astra.ses.spell.language.model.ast.UnaryOp;
...
/**
 * Provides the basic behavior for a tree builder (opening and closing node
 * scopes).
 * 
...
public abstract class AbstractTreeBuilder extends AbstractTreeBuilderHelpers
{
	 ...
	 * Constructor
	 */
	public AbstractTreeBuilder(AbstractJJTGrammarState stack)
	{
		super(stack);
	}

	/**
	 * Subclasses must implement this method to deal with any node that's not
	 * properly handled in this base.
	 * 
	 * @param n
	 *            the node that should be closed
	 * @param arity
	 *            the current number of nodes in the stack (found after the
	 *            context was opened)
	 * @return a new node representing the node that's having it's context
	 *         closed.
	 * @throws Exception
	 */
	protected abstract SimpleNode onCloseNode(SimpleNode n, int arity)
	        throws Exception;

	/**
	 * Opens a new scope and returns a node to be used in this scope. This same
	 * node will later be called in {@link #closeNode(SimpleNode, int)} to have
	 * its scope closed (and at that time it may be changed for a new node that
	 * represents the scope more accurately.
	 */
	public final SimpleNode openNode(int id)
	{
		...
		case JJTNAME:
		case JJTDOTTED_NAME:
			// the actual name will be set during the parsing (token image) --
			// see Name construct
			ret = new Name(null, Name.Load, false);
			break;

		case JJTNUM:// the actual number will be set during the parsing (token
					// image) -- see Num construct
		...
		case JJTUNICODE:
		case JJTBINARY:
			// the actual number will be set during the parsing (token image) --
			// see Num construct
	...
	public SimpleNode closeNode(SimpleNode n, int arity) throws Exception
	{
		...
		case JJTCOMMA:
		case JJTCOLON:
			return n; // it's already the correct node (and it's value is
		...
		case JJTDECORATORS:
			...
			while (stack.nodeArity() > 0)
			{
				...
				while (!(node instanceof decoratorsType))
				{
					...
					{
						listArgs.add(((ComprehensionCollection) node)
						        .getGenerators()[0]);
						listArgs.add(stack.popNode()); // target
			...
			return new Decorators(
			        (decoratorsType[]) list2.toArray(new decoratorsType[0]),
			        JJTDECORATORS);
		...
		case JJTSUBSCRIPTLIST:
			...
			for (int i = arity - 1; i >= 0; i--)
			{
				...
				else if (sliceNode instanceof IdentityNode)
				{
					...
					// this happens when parsing something like a[1,], whereas
					// a[1,2] would not have this.

				}
				else
				{
					throw new RuntimeException(
					        "Expected a sliceType or an IdentityNode. Received :"
					                + sliceNode.getClass());
				}
			}
		...
		case JJTDOTTED_NAME:
			...
			for (int i = 0; i < arity; i++)
			{
				...
				// we have to set that, because if we later add things to the
				// previous Name, we will now want it to be added to
				// the new name (comments will only appear later and may be
				// added to the previous name -- so, we replace the previous
				// name specials list).
		...
		// if we found a node not expected in the base, let's give subclasses an
	 ...
	 * 
	 * @param n
	 *            the node that opened the if scope.
	 * @param arity
	 *            the current number of nodes in the stack.
